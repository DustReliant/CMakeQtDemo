...
QTableViewDelegate::QTableViewDelegate(QObject *parent) : QStyledItemDelegate(parent)
{
    delegatePwd = false;
    delegateEdit = false;
    delegateEnable = true;
...
bool QTableViewDelegate::eventFilter(QObject *object, QEvent *event)
{
...
int QTableViewDelegate::getLevel(const QModelIndex &index) const
{
    //找到当前数据模型所在层级
    int level = 0;
...
void QTableViewDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index)const
{
    //设置应用委托最小的层级,比如QTreeWidget
    if (getLevel(index) < delegateLevel) {
        return;
...
void QTableViewDelegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //不执行这个生成的控件在坐标原点
    editor->setGeometry(option.rect);
}

bool QTableViewDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    //设置应用委托最小的层级,比如QTreeWidget
    if (getLevel(index) < delegateLevel) {
...
void QTableViewDelegate::drawBg(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &) const
{
...
#ifdef quistyle
    hoverBgColor2 = QColor(QUIConfig::DarkColorEnd);
    selectBgColor2 = QColor(QUIConfig::NormalColorEnd);
#endif
...
void QTableViewDelegate::drawCheckBox(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void QTableViewDelegate::drawPushButton(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //没有按钮对应的值则不处理
    QString data = index.data(Qt::DisplayRole).toString();
    QStringList list = data.split("|");
...
void QTableViewDelegate::drawProgressBar(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //处理进度条委托
...
void QTableViewDelegate::drawColor(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //取出颜色值进行绘制
...
void QTableViewDelegate::drawLineEdit(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //处理密码委托,将明文转为密文显示
    QString data = index.model()->data(index, Qt::DisplayRole).toString();
    int len = data.length();
...
void QTableViewDelegate::drawPixmap(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void QTableViewDelegate::drawDisable(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    //继续父类的绘制
...
QString QTableViewDelegate::getKeyValue(const QString &data, bool key) const
{
    //根据关键字对照表分别绘制成设定的文本
...
void QTableViewDelegate::drawKey(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    QString data = index.data(Qt::DisplayRole).toString();
    QString value = getKeyValue(data, true);
    if (value.isEmpty()) {
...
void QTableViewDelegate::checkData(const QString &data, bool *exist) const
{
    *exist = false;
    if (checkType == "==") {
        if (data == checkValue) {
...
void QTableViewDelegate::drawText(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (!checkText) {
        return;
    }
...
void QTableViewDelegate::slot_valueChanged()
{
    emit valueChanged(checkedText);
}

void QTableViewDelegate::slot_valueChanged(int value)
{
...
void QTableViewDelegate::setDelegatePwd(bool delegatePwd)
{
    this->delegatePwd = delegatePwd;
}

void QTableViewDelegate::setDelegateEdit(bool delegateEdit)
{
    this->delegateEdit = delegateEdit;
}

void QTableViewDelegate::setDelegateEnable(bool delegateEnable)
{
    this->delegateEnable = delegateEnable;
}

void QTableViewDelegate::setDelegateMin(int delegateMin)
{
    this->delegateMin = delegateMin;
}

void QTableViewDelegate::setDelegateMax(int delegateMax)
{
    this->delegateMax = delegateMax;
}

void QTableViewDelegate::setDelegateColumn(int delegateColumn)
{
    this->delegateColumn = delegateColumn;
}

void QTableViewDelegate::setDelegateLevel(int delegateLevel)
{
    this->delegateLevel = delegateLevel;
}

void QTableViewDelegate::setDelegateType(const QString &delegateType)
{
    //设置了对象名称方便设置全局样式
...
void QTableViewDelegate::setDelegateKey(const QStringList &delegateKey)
{
    this->delegateKey = delegateKey;
}

void QTableViewDelegate::setDelegateValue(const QStringList &delegateValue)
{
    this->delegateValue = delegateValue;
}

void QTableViewDelegate::setCheckBoxText(const QString &checkBoxChecked, const QString &checkBoxUnchecked)
{
    this->checkBoxChecked = checkBoxChecked;
    this->checkBoxUnchecked = checkBoxUnchecked;
}

void QTableViewDelegate::setCheckColumn(int checkColumn)
{
    this->checkColumn = checkColumn;
}

void QTableViewDelegate::setCheckType(const QString &checkType)
{
    this->checkType = checkType;
}

void QTableViewDelegate::setCheckValue(const QString &checkValue)
{
    this->checkValue = checkValue;
}

void QTableViewDelegate::setCheckOkImage(const QPixmap &checkOkImage)
{
    this->checkOkImage = checkOkImage;
}

void QTableViewDelegate::setCheckNoImage(const QPixmap &checkNoImage)
{
    this->checkNoImage = checkNoImage;
}

void QTableViewDelegate::setCheckImageWidth(int checkImageWidth)
{
    this->checkImageWidth = checkImageWidth;
}

void QTableViewDelegate::setCheckImageHeight(int checkImageHeight)
{
    this->checkImageHeight = checkImageHeight;
}

void QTableViewDelegate::setCheckText(bool checkText)
{
    this->checkText = checkText;
}

void QTableViewDelegate::setCheckTextColor(const QColor &checkTextColor)
{
    this->checkTextColor = checkTextColor;
}

void QTableViewDelegate::setCheckBgColor(const QColor &checkBgColor)
{
    this->checkBgColor = checkBgColor;
}

void QTableViewDelegate::setTextColor(const QColor &textColor)
{
    this->textColor = textColor;
}

void QTableViewDelegate::setHoverBgColor(const QColor &hoverBgColor)
{
    this->hoverBgColor = hoverBgColor;
}

void QTableViewDelegate::setSelectBgColor(const QColor &selectBgColor)
{
